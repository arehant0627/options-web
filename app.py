...
# ─────────────────────────────────────────────── Screener
with tab1:
    # Guard against a stale results frame (old core.py, or a cached run from
    # before an upgrade) that predates the put columns.
    is_put = ("side" in df.columns) and (df["side"].iloc[0] == "put")
    ...
    missing = [c for c in MAIN if c not in df.columns]
    MAIN = [c for c in MAIN if c in df.columns]
    if missing:
        st.warning(f"{len(missing)} column(s) missing from these results: "
                   f"`{'`, `'.join(missing)}`. That means app.py and core.py are "
                   "out of sync — re-upload core.py, then reboot and re-run.")
